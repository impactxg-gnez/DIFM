...
const VALID_TRANSITIONS: Record<JobStatus, JobStatus[]> = {
  REQUESTED: ['PRICED', 'REVIEW_REQUIRED'],
  REVIEW_REQUIRED: ['PRICED', 'COLLECTING_QUOTES', 'ASSIGNED', 'CANCELLED_FREE', 'CANCELLED_CHARGED'],
  PRICED: ['BOOKED'],
  BOOKED: ['WAITING_FOR_DISPATCH', 'ASSIGNING'],
  WAITING_FOR_DISPATCH: ['ASSIGNING'],
  ASSIGNING: ['ASSIGNED', 'RESCHEDULE_REQUIRED', 'FLAGGED_REVIEW'],
  COLLECTING_QUOTES: ['ASSIGNED', 'CANCELLED_FREE', 'CANCELLED_CHARGED'],
  ASSIGNED: ['PREAUTHORISED', 'FLAGGED_REVIEW'],
  PREAUTHORISED: ['ARRIVING'],
  ARRIVING: ['ARRIVED', 'ISSUE_RAISED_BY_PROVIDER'],
  ARRIVED: ['IN_PROGRESS', 'COMPLETED', 'MISMATCH_PENDING', 'PARTS_REQUIRED', 'PARTS_PENDING_APPROVAL'],
  IN_PROGRESS: ['COMPLETED', 'MISMATCH_PENDING', 'PARTS_REQUIRED', 'PARTS_PENDING_APPROVAL', 'ISSUE_RAISED_BY_PROVIDER'],
  MISMATCH_PENDING: ['ASSIGNED', 'ARRIVED', 'IN_PROGRESS', 'REBOOK_REQUIRED'],
  ...
  PARTS_PENDING_APPROVAL: ['IN_PROGRESS'],
...
const NON_TERMINAL_STATES: JobStatus[] = [
  'REQUESTED', 'REVIEW_REQUIRED', 'PRICED', 'BOOKED', 'WAITING_FOR_DISPATCH', 'ASSIGNING', 'COLLECTING_QUOTES', 'ASSIGNED',
  'PREAUTHORISED', 'ARRIVING', 'ARRIVED', 'IN_PROGRESS', 'SCOPE_MISMATCH', 'MISMATCH_PENDING', 'REBOOK_REQUIRED', 'PARTS_REQUIRED',
