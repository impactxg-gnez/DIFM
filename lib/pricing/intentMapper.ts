...
export const RULES: DeterministicRule[] = [
    ...
    { match: ['light', 'lights'], job: 'light_install' },
...
const RULE_JOB_ALIASES: Record<string, string[]> = {
    replace_socket: ['replace_socket', 'replace_socket_faceplate', 'socket_replace'],
    light_install: ['light_install', 'install_light_fitting', 'install_light_fitting_standard'],
...
export function getClarifiers(
    ...
    for (const entry of jobs) {
        if (entry.job === 'tv_mount_standard') {
            if (tvDetails.size === null) {
                addClarifier({ tag: 'TV_SIZE', question: 'What is the TV size in inches?', affects_time: true });
            }
            if (!tvDetails.wall) {
                addClarifier({ tag: 'WALL_TYPE', question: 'What wall type is it mounted on?', affects_time: true });
            }
            if (tvDetails.concealed === null) {
                addClarifier({ tag: 'CABLE_CONCEALMENT', question: 'Do you need cables concealed?', affects_time: true });
            }
            continue;
        }
...
export function mapToJobs(input: string, allowedJobIds: string[]): IntentMappingResult {
    ...
    for (const part of parts) {
        ...
        const resolvedJobId = resolveAllowedJobId(mapped.job, allowedSet);
